...
- **GGUF 导入** — 导入本地 GGUF 模型文件，基于 blob 上传机制。自动检测并分组分片文件（如 `model-00001-of-00004.gguf`）。支持批量导入和进度追踪。
- **模型删除** — 删除前弹出确认对话框，显示模型名称和大小。
- **版本检测** — 在状态栏显示 Ollama 服务器版本号。若版本低于最低要求（v0.5.0），显示警告横幅并提供一键跳转升级链接。
- **设置** — 配置 Ollama 主机地址/端口和模型存储目录。一键复制 API 地址。查看和打开日志文件。
- **系统托盘** — 常驻菜单栏（macOS）或系统托盘（Windows）。图标颜色反映 Ollama 状态：绿色 = 运行中，灰色 = 已停止。
- **原生外观** — macOS 毛玻璃效果（vibrancy），Windows Mica 材质。自动跟随系统深色/浅色模式。
- **日志系统** — 内置文件日志系统，便于调试。日志按天存储在应用日志目录中。
...
- 系统需已安装 [Ollama](https://ollama.com/download) v0.5.0 或更高版本
...
```
src/
├── shared/              # 共享类型、IPC 通道常量、MIN_OLLAMA_VERSION
├── main/
│   ├── index.ts         # 托盘 + 窗口管理
│   ├── logger.ts        # 文件日志系统
│   ├── ipc/handlers.ts  # IPC 处理器注册
│   ├── ollama/
│   │   ├── api.ts       # Ollama REST API 客户端（NDJSON 流、blob 上传）
│   │   ├── service.ts   # 进程管理（macOS + Windows）
│   │   └── status-poller.ts  # 健康检查 + 版本轮询
│   └── store.ts         # electron-store 配置
├── preload/             # contextBridge（类型安全 API）
└── renderer/src/
    ├── components/      # ServiceStatus, ModelList, PullModelInput, Settings, VersionWarning 等
